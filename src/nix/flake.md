...
* `sourcehut`: Similar to `github`, is a more efficient way to fetch
  ...
  * `repo`: The name of the repository.
  ...
  a `param`, to point to instances other than `git.sr.ht`.

  This currently only works for Git repositories. So, for Mercurial
  repositories (including `hg.sr.ht`), you should use `mercurial` instead.
